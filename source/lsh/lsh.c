...
static void parse_args(int argc, char** argv, char* input_file_name, char* query_file_name, char* output_file_name, int* k, int* L, int* N, double* R){
...
int main(int argc, char** argv){
    ...
    while(1){
        ...
        int k = 4, L = 5, N = 1, w = W;    // is not given as arguments from the user
        
        uint32_t magic_number = 0, num_of_images = 0, num_of_rows = 0, num_of_cols = 0; 

        /**** Input parsing ****/

        parse_args(argc, argv, input_file_name, query_file_name, output_file_name, &k, &L, &N, &R);
        ...
        for(int i = 0; i < L; i++){
            bucket[i] = (int*) mem_allocate(sizeof(int) * query_size);
        }
        for(int a = 0; a < L; a++){
            for(int i = 0; i < query_size; i++){
                random_h_values(h_array, q_array[i], get_v_values(random_values[a]), get_t_values(random_values[a]), k, w, vector_size);
                uint32_t id = random_id_values(h_array, get_r_values(random_values[a]), k);
        ...
        List* kNN = lsh_approximate_kNN(hashtables, q_array, bucket, L, N, vector_size, query_size, kNN_start_time, kNN_end_time);
        
        /**** Approximate Range Search ****/

        List* range = lsh_approximate_range_search(hashtables, q_array, bucket, L, N, vector_size, query_size, R);
        ...
        if((strcmp("Y", compare_question) == 0) || (strcmp("y", compare_question) == 0)){
            FILE* file = fopen("../../data/lsh_compare.csv", "w+");
            if(file == NULL){
        ...
        fprintf(output_file, "tAverageLSH  : %f\n", total_kNN / query_size);
